...
################----ФУНКЦИЯ ПОРОГОВ ОЦЕНКИ ПРОИЗНОШЕНИЯ----------#########
async def analyze_pronunciation(
    ...
    # Базовые пороговые значения и логика расчета (как в оригинальной функции)
    ...
    try:
        ...
        # Вызываем callback-функцию с результатами
        await callback(overall_accuracy, verdict, analysis_text)
    ...
    if audio_path and os.path.exists(audio_path):
        try:
            ...
            await message.answer_voice(
                voice=FSInputFile(audio_path),  # Основное изменение здесь
...
async def process_pronunciation_recording(message: Message, state: FSMContext):
    """Обработка записи произношения слова"""
    data = await state.get_data()
    ...
    async def handle_result(accuracy, verdict, analysis_text):
        await message.answer(
...
async def slow_down_true_false(callback: CallbackQuery, state: FSMContext):
    ...
    if audio_path and os.path.exists(audio_path):
        try:
            ...
            await callback.message.answer_voice(
                audio,
                caption="🎧 **Прослушайте фразу (медленно)**",
                parse_mode="Markdown"
            )
        except Exception as e:
            print(f"Ошибка отправки аудио: {e}")
...
async def slow_down_listening_choice(callback: CallbackQuery, state: FSMContext):
        ...
        if audio_path and os.path.exists(audio_path):
            try:
                ...
                audio_msg = await callback.message.answer_voice(
                    audio,
                    caption="🎧 **Прослушайте фразу 2 раза (медленно)**",
                    parse_mode="Markdown"
                )
            except Exception as e:
                print(f"Ошибка отправки аудио: {e}")
...
async def slow_down_listening_phrases(callback: CallbackQuery, state: FSMContext):
...
async def request_speaking_recording(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "🎤 **Запиши голосовое сообщение со своми мыслыми по те ме.**\n\n"
        "💡 Говори свободно на английском языке. Можешь рассказать о своем опыте, "
        "привести примеры из работы или поделиться мнением.\n\n"
        "Для записи нажмите на микрофон в нижнем правом углу  в Telegram и начните говорить.",
        parse_mode="Markdown",
        reply_markup=get_keyboard_with_menu(get_speaking_keyboard())
    )
...
async def process_speaking_recording(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            f"**Что вы сказали:** {transcribed_text}\n\n"
            f"{analysis}",
            parse_mode="Markdown",
